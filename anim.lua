-- Taken from https://github.com/tdubourg/Watch-your-back/, thanks Jean-Marie!
Anim = {}
Anim.__index = Anim
...
Anim.DELAY = 100  -- toutes les 200ms, on fait anim.next()
...
-- PUBLIC : constructor
function Anim.new(folder, spriteWidth, spriteHeight)
    local self = {}
    ...
    for key,val in pairs(Anim.ANIMS) do
        ...
        for i=1, val.number do
            local path = 'images/anim/'..folder..'/'..key..'/'..i..'.png'
            -- print("loading image =>", path)
            print ("\n\n\n\n\n\n")
            print ("Loading: " .. path)
            print ("\n\n\n\n\n\n")
            self.sprites[key][i] = display.newImageRect(path, spriteWidth, spriteHeight)
            self.sprites[key][i].isVisible = false
        end
    end
...
function Anim:updateImg()
    if self.currentImg then
        self.currentImg.isVisible = false
    end
